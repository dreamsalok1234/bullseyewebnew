...
export class CommonService {
  ...
  getTicker1DDataListByType(chatSymbol, type, currency, limit, callback) {
    ...
    let apiName = '';
    if (type.toLowerCase() === 'crypto' || type.toLowerCase() === 'cryptocurrency') {
      apiName = 'https://min-api.cryptocompare.com/data/histominute?fsym=' + chatSymbol + '&tsym=' + currency + '&limit=100';
    } else {
      apiName = 'https://intraday.worldtradingdata.com/api/v1/intraday?symbol=' + chatSymbol + '&range=1&interval=1&api_token=Enbl13NdXIwtInHQM4u9wNQMXrBEJY3WQnqb1nBlT8vK7aWK9DUXdKlcJLVc';
      // apiName += '&symbol=' + chatSymbol;
    }
    this.globalService.callGlobalGetApi(apiName, false).subscribe(data => {
      try {
